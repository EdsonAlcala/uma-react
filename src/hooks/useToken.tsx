...
export const useToken = (empAddress: EthereumAddress, address?: EthereumAddress, tokenAddress?: EthereumAddress): TokenData | undefined => {
    // external
    const { block$ } = useWeb3Provider()
    const { instance } = useERC20At(tokenAddress)

    // state
    const [tokenState, setTokenState] = useState<TokenData | undefined>(undefined)
    ...
    const getAllowance = async (contractInstance: ethers.Contract, addressParam: EthereumAddress, newDecimals: number) => {
        const allowanceRaw = await contractInstance.allowance(addressParam, empAddress)
    ...
    useEffect(() => {
        if (block$ && instance) {
            const sub = block$.subscribe(() => getTokenInfo(instance).catch((error) => console.log('Error getTokenInfo', error)))
